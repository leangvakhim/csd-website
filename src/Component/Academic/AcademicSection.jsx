...
const AcademicSection = ({ section, menuLang }) => {
  ...
  // Process academics info from globalData
  useEffect(() => {
    if (globalData?.academics) {
      const items = globalData.academics;
      const item = items.find((entry) => entry.acad_sec === section.sec_id);
      if (item) {
        const details = item.acad_detail.split("\n\n");
        const resolvedRoute = getAlias(item.acad_routepage);
        setAcademics({
  ...
  }, [section.sec_id, globalData?.academics, globalData?.pages]);
